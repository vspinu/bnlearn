...
/* R interface for Dirichlet posterior probabilities (covers BDe and K2 scores). */
SEXP dirichlet_node(SEXP target, SEXP x, SEXP data, SEXP iss, SEXP prior,
    SEXP beta, SEXP experimental, SEXP sparse, SEXP debug) {

int *debuglevel = LOGICAL(debug);
char *t = (char *)CHAR(STRING_ELT(target, 0));
double *pb = NULL, *pr = NULL;
SEXP nodes, node_t, data_t, exp_data, parents, parent_vars, config;
SEXP prior_prob, prob;
  ...
  if (LENGTH(parents) == 0) {
    ...
    *pb = dpost(data_t, iss, exp_data);
    ...
    PROTECT(config = c_cfg2(parent_vars, TRUE, !isTRUE(sparse)));
    /* compute the marginal likelihood. */
    *pb = cdpost(data_t, config, iss, exp_data);
